...
class TestDatabaseRuntimeAndAuth(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_engine_verify_database_readiness_rejects_mismatched_revision(self):
        """Verify that readiness check rejects versions other than 0017."""
        ...
        try:
            with self.assertRaises(RuntimeError) as ctx:
                await verify_database_readiness(async_eng)
            self.assertIn("expected revision '0017'", str(ctx.exception))
        finally:
            self.su_cur.execute("UPDATE public.alembic_version SET version_num = '0017'")
            await async_eng.dispose()
    ...
    async def test_health_readiness_mismatch_revision_returns_503(self):
        ...
        response = MagicMock()
        with patch("api.routes.health.verify_database_readiness", side_effect=RuntimeError("Database Migration Mismatch: expected revision '0017', got '0008'")):
            res = await readiness_check(response=response)
