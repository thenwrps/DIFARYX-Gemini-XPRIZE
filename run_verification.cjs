...
const tempCsvPath = path.join(__dirname, 'temp_upload_xrd.xy');
...
(async () => {
  ...
  const ensureAuthenticated = async () => {
    ...
    if (url.includes('/signin')) {
      console.log('Redirected to sign-in page. Clicking guest login...');
      await page.click('button:has-text("Continue as Guest")');
      await page.waitForTimeout(2000);
      console.log('Auth check complete. Current URL:', page.url());
    } else {
      console.log('Already authenticated. Current URL:', url);
    }
  };
  ...
  try {
    ...
    await page.goto('http://localhost:5173/workspace/xrd?project=cufe2o4-sba15&mode=demo');
    await page.waitForTimeout(3000);
    
    const xrdStorageBefore = await getStorage();
    console.log('XRD before reprocess: overrides key exists:', Boolean(xrdStorageBefore['difaryx-parameter-state:v2:cufe2o4-sba15:xrd']));

    // Switch to Parameters tab first
    console.log('Switching to Parameters tab...');
    await page.click('button:text("Parameters")');
    ...
    console.log('XRD after reprocess: overrides key exists:', Boolean(xrdStorageAfter['difaryx-parameter-state:v2:cufe2o4-sba15:xrd']));
    await page.screenshot({ path: path.join(ARTIFACT_DIR, '03_workspace_xrd.png') });

    // 4. FTIR Workspace
    console.log('\n--- Step 4: FTIR Workspace ---');
    await page.goto('http://localhost:5173/workspace/ftir?project=cufe2o4-sba15&mode=demo');
    await page.waitForTimeout(2000);
    ...
    await page.goto('http://localhost:5173/workspace/raman?project=cufe2o4-sba15&mode=demo');
    await page.waitForTimeout(2000);
    // Switch to Parameters tab first
    console.log('Switching to Parameters tab...');
    await page.click('button:text("Parameters")');
    await page.waitForTimeout(1000);
    console.log('Clicking Detect Modes...');
    await page.click('button:has-text("Detect Modes")');
    await page.waitForTimeout(1500);
    await page.screenshot({ path: path.join(ARTIFACT_DIR, '05_workspace_raman.png') });

    // 6. XPS Workspace
    console.log('\n--- Step 6: XPS Workspace ---');
    await page.goto('http://localhost:5173/workspace/xps?project=cufe2o4-sba15&mode=demo');
    await page.waitForTimeout(2000);
    // Switch to Parameters tab first
    console.log('Switching to Parameters tab...');
    await page.click('button:text("Parameters")');
    await page.waitForTimeout(1000);
    console.log('Clicking Fit Region...');
    await page.click('button:has-text("Fit Region")');
    await page.waitForTimeout(1500);
    await page.screenshot({ path: path.join(ARTIFACT_DIR, '06_workspace_xps.png') });

    // 7. Fusion Workspace
    console.log('\n--- Step 7: Fusion Workspace ---');
    await page.goto('http://localhost:5173/workspace/fusion?project=cufe2o4-sba15');
    await page.waitForTimeout(3000);
    await page.screenshot({ path: path.join(ARTIFACT_DIR, '07_workspace_fusion_empty.png') });
    
    console.log('Clicking Run Fusion...');
    await page.click('button:has-text("Run Fusion")');
    await page.waitForTimeout(2000);
    ...
    await page.click('button:has-text("Cross-Technique Insights")', { force: true });
    await page.waitForTimeout(1000);
    await page.screenshot({ path: path.join(ARTIFACT_DIR, '07_workspace_fusion_insights.png') });
    
    await page.click('button:has-text("Review Cards")', { force: true });
    await page.waitForTimeout(1000);
    await page.screenshot({ path: path.join(ARTIFACT_DIR, '07_workspace_fusion_review.png') });

    await page.click('button:has-text("Contradictions")', { force: true });
    await page.waitForTimeout(1000);
    ...
    await page.goto('http://localhost:5173/workspace/multi?project=cufe2o4-sba15');
    await page.waitForTimeout(3000);
    ...
    await page.goto('http://localhost:5173/demo/agent?project=cufe2o4-sba15&mode=demo');
    await page.waitForTimeout(2000);
    ...
    if (entries.length > 0) {
      ...
      console.log('Last notebook entry title:', lastEntry.title);
      console.log('Last notebook entry subtitle:', lastEntry.subtitle);
      if (lastEntry.sections && lastEntry.sections.length > 0) {
        console.log('Last notebook entry first section lines:', lastEntry.sections[0].lines.slice(0, 2));
      }
    }
    ...
    await page.goto('http://localhost:5173/reports?project=cufe2o4-sba15');
    await page.waitForTimeout(3000);
